...
final class PersonalJuly2026ScenarioTests: XCTestCase {
    ...
    func testAfterICloudUsesScheduledProductionWorkflowOnce() async throws {
        ...
        XCTAssertEqual(store.snapshot.pots.first { $0.id == "pot-barclays" }?.balancePence, barclaysBefore)
        XCTAssertEqual(store.snapshot.pots.reduce(0) { $0 + $1.balancePence }, totalBefore)
